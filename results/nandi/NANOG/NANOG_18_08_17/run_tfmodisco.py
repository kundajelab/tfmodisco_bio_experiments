...
if len(sys.argv) < 5 or len(sys.argv) > 6:
    print("Syntax: ", sys.argv[0] , " <score prefix> <sequence fa file> <sequence tsv> <seed> {<number of tasks> | <start task> <end task>}")
    quit()
...
if len(sys.argv) == 6:
    ...
    start_task    = int(sys.argv[5])            # 
    end_task    = int(sys.argv[6])            # 
...
factory = modisco.tfmodisco_workflow.seqlets_to_patterns.TfModiscoSeqletsToPatternsFactory(
              ...
              num_mismatches=2,
              final_min_cluster_size=30, seed=seed)
